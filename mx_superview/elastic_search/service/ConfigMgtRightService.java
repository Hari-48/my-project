...
import com.finsurge.tmr_portal.mx_superview.elastic_search.entity.ConfigMgtRight;
...
@Service
public class ConfigMgtRightService {
    private static final Logger log = LoggerFactory.getLogger(ConfigMgtRightService.class);
    @Autowired
    ...
    public ConfigMgtRightService(SearchHistoryRepository searchHistoryRepository, Environment environment, ElasticsearchClient esClient) {
        this.searchHistoryRepository = searchHistoryRepository;
        this.environment = environment;
        this.esClient = esClient;
    }

    public Document getConfigMgtRightList(String date, ElasticSearchModel elasticSearchModel, String sortBy, String sortingOrder,
                                          int pageSize, boolean isGlobalSearch, String bulkFilterSearchType, String username) throws IOException, ParseException {

        Document configMgtRightList = new Document();
        List<Query> queryList = new ArrayList<>();
        String indexName = ConfigMgtRight.INDEX_NAME;

        // Fetching Data From  JSON File .
        List<String> fields = elastic_utils.getDataFromJsonFile("elastic/GlobalSearchFields/configMgtRight.json", "fields");
        //Fetching IntegerFields From  JSON File .
        List<String> integerFields = elastic_utils.getDataFromJsonFile("elastic/GlobalSearchFields/configMgtRight.json", "integerFields");
        //Fetching Global Search Fields From  JSON File .
        List<String> globalSearchFields = elastic_utils.getDataFromJsonFile("elastic/GlobalSearchFields/configMgtRight.json", "globalSearchFields");
        ...
        if (elasticSearchModel.getFilterColumnName() != null && !elasticSearchModel.getFilterColumnName().isEmpty()) {
            ...
            SearchResponse<ConfigMgtRight> searchResponse = esClient.search(searchRequest, ConfigMgtRight.class);
            ...
            dropDownFieldCount = elastic_utils.getDropDownFieldCount(elasticSearchModel.getFilterColumnName(), queryList, indexName, ConfigMgtRight.class);
            ...
            configMgtRightList.put("filterValues", newDropDown);
            configMgtRightList.put("dropDownFieldCount", dropDownFieldCount);
            configMgtRightList.put("filterColumnNumericCount", filterColNumericCount);
            configMgtRightList.put("filterColumnAlphaNumericCount", filterColAlphaNumericCount);
        } else {
            List<ConfigMgtRight> allDocuments = new ArrayList<>();
            ...
            String template = "CONFIGURATION_MANAGEMENT_RIGHTS_TEMPLATE";
            ...
            configMgtRightList.put("totalPage", totalPages);
            configMgtRightList.put("count", totalCount);
            configMgtRightList.put("pageScroll", pageScroll);
            configMgtRightList.put("content", allDocuments);
            configMgtRightList.put("sorting", document);
            configMgtRightList.put("pitId", pitId);
        }
        return configMgtRightList;
    }


    @Async
    @Transactional
    public CompletableFuture<Void> getConfigMgtRightListExport(String reportDate, String fileName, List<FieldMap> fieldMaps, String colour, List<String> fieldColumns, DownloadJob job, String outputFormat, String sortBy, String sortingOrder, String userName, int pageSize, boolean globalSearch, String bulkFilterSearchType, ElasticSearchModel elasticSearchModel) throws Exception {
        List<ConfigMgtRight> configMgtRightList = new ArrayList<>();
        ...
        for (int i = 0; i < totalPage; i++) {
            ...
            Document searchConfigMgtRight = getConfigMgtRightList(reportDate,
            ...
            configMgtRightList.addAll((Collection<? extends ConfigMgtRight>) searchConfigMgtRight.get("content"));
            docs = (Document) searchConfigMgtRight.get("sorting");

            downloadJobService.updateJobProgress("FETCH_ITEMS", (int) totalCount, configMgtRightList.size(), job.getId());
            totalPage = (long) searchConfigMgtRight.get("totalPage");
            log.info("TOTAL PAGE {}", totalPage);
            totalCount = (long) searchConfigMgtRight.get("count");
            log.info("TOTAL COUNT {}", totalCount);
        }
        log.info(" Export: Fetch complete.");
        downloadJobService.updateJobProgress("FILE_WRITE", configMgtRightList.size(), (int) totalCount, job.getId());
        return elastic_utils.exportFileFromDetails(outputFormat, configMgtRightList, fileName, fieldMaps, colour, job, fieldColumns, elasticSearchModel);
    }
