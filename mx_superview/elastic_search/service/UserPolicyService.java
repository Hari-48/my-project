...
import com.finsurge.tmr_portal.mx_superview.elastic_search.entity.UserPolicy;
...
public class UserPolicyService {
    private static final Logger log = LoggerFactory.getLogger(UserPolicyService.class);
    @Autowired
    ...
    public UserPolicyService(SearchHistoryRepository searchHistoryRepository, Environment environment, ElasticsearchClient esClient) {
    ...
    @SneakyThrows
    public Document getUserPolicyData(String date, ElasticSearchModel elasticSearchModel, String sortBy, String sortingOrder,
                                      int pageSize, boolean isGlobalSearch, String bulkFilterSearchType, String username) {

        Document userPolicyList = new Document();
        List<Query> queryList = new ArrayList<>();
        String indexName = UserPolicy.INDEX_NAME;

        List<String> fields = elastic_utils.getDataFromJsonFile("elastic/GlobalSearchFields/userPolicy.json", "fields");
        List<String> integerFields = elastic_utils.getDataFromJsonFile("elastic/GlobalSearchFields/userPolicy.json", "integerFields");
        List<String> globalSearchFields = elastic_utils.getDataFromJsonFile("elastic/GlobalSearchFields/userPolicy.json", "globalSearchFields");
        ...
        if (elasticSearchModel.getFilterColumnName() != null && !elasticSearchModel.getFilterColumnName().isEmpty()) {
            ...
            SearchResponse<UserPolicy> searchResponse = esClient.search(searchRequest, UserPolicy.class);
            ...
            dropDownFieldCount = elastic_utils.getDropDownFieldCount(elasticSearchModel.getFilterColumnName(), queryList, indexName, UserPolicy.class);
            ...
            userPolicyList.put("filterValues", newDropDown);
            userPolicyList.put("dropDownFieldCount", dropDownFieldCount);
            userPolicyList.put("filterColumnNumericCount", filterColNumericCount);
            userPolicyList.put("filterColumnAlphaNumericCount", filterColAlphaNumericCount);
        } else {
            List<UserPolicy> allDocuments = new ArrayList<>();
            ...
            SortOptions sortOptions2 = SortOptions.of(s ->
                    s.field(FieldSort.of(fs -> fs.
                            field("mname.keyword").order(sortingOrder.equalsIgnoreCase("asc")
            ...
            userPolicyList.put("totalPage", totalPages);
            userPolicyList.put("count", totalCount);
            userPolicyList.put("pageScroll", pageScroll);
            userPolicyList.put("content", allDocuments);
            userPolicyList.put("sorting", document);
            userPolicyList.put("pitId", pitId);
        }
    ...
    public CompletableFuture<Void> getUserPolicyExport(String reportDate, String fileName, List<FieldMap> fieldMaps, String colour, List<String> fieldColumns, DownloadJob job, String outputFormat, String sortBy, String sortingOrder, String userName, int pageSize, boolean globalSearch, String bulkFilterSearchType, ElasticSearchModel elasticSearchModel) throws Exception {
        ...
        for (int i = 0; i < totalPage; i++) {
            ...
            Document userPolicyValues = getUserPolicyData(reportDate,
            ...
            userPolicyList.addAll((Collection<? extends UserPolicy>) userPolicyValues.get("content"));
            docs = (Document) userPolicyValues.get("sorting");

            downloadJobService.updateJobProgress("FETCH_ITEMS", (int) totalCount, userPolicyList.size(), job.getId());
            totalPage = (long) userPolicyValues.get("totalPage");
            log.info("TOTAL PAGE {}", totalPage);
            totalCount = (long) userPolicyValues.get("count");
            log.info("TOTAL COUNT {}", totalCount);

        }
        log.info(" Export: Fetch complete.");
        downloadJobService.updateJobProgress("FILE_WRITE", userPolicyList.size(), (int) totalCount, job.getId());
        return elastic_utils.exportFileFromDetails(outputFormat, userPolicyList, fileName, fieldMaps, colour, job, fieldColumns, elasticSearchModel);
    }
