...
import com.finsurge.tmr_portal.mx_superview.elastic_search.entity.EnterpriseRisk;
...
public class EnterpriseRiskService {
    private static final Logger log = LoggerFactory.getLogger(EnterpriseRiskService.class);
    @Autowired
    ...
    public EnterpriseRiskService(SearchHistoryRepository searchHistoryRepository, Environment environment, ElasticsearchClient esClient) {
        this.searchHistoryRepository = searchHistoryRepository;
        this.environment = environment;
        this.esClient = esClient;
    }

    public Document getEnterpriseRiskList(String date, ElasticSearchModel elasticSearchModel, String sortBy, String sortingOrder,
                                          int pageSize, boolean isGlobalSearch, String bulkFilterSearchType, String username) throws IOException, ParseException {

        Document EnterpriseRiskList = new Document();
        List<Query> queryList = new ArrayList<>();
        String indexName = EnterpriseRisk.INDEX_NAME;

        // Fetching Data From  JSON File .
        List<String> fields = elastic_utils.getDataFromJsonFile("elastic/GlobalSearchFields/enterpriseRisk.json", "fields");
        //Fetching IntegerFields From  JSON File .
        List<String> integerFields = elastic_utils.getDataFromJsonFile("elastic/GlobalSearchFields/enterpriseRisk.json", "integerFields");
        //Fetching Global Search Fields From  JSON File .
        List<String> globalSearchFields = elastic_utils.getDataFromJsonFile("elastic/GlobalSearchFields/enterpriseRisk.json", "globalSearchFields");
        ...
        if (elasticSearchModel.getFilterColumnName() != null && !elasticSearchModel.getFilterColumnName().isEmpty()) {
            ...
            SearchResponse<EnterpriseRisk> searchResponse = esClient.search(searchRequest, EnterpriseRisk.class);
            ...
            dropDownFieldCount = elastic_utils.getDropDownFieldCount(elasticSearchModel.getFilterColumnName(), queryList, indexName, EnterpriseRisk.class);
            ...
            EnterpriseRiskList.put("filterValues", newDropDown);
            log.info("DropDown Count------------Cardinality :{}", dropDownFieldCount);
            EnterpriseRiskList.put("dropDownFieldCount", dropDownFieldCount);
            EnterpriseRiskList.put("filterColumnNumericCount", filterColNumericCount);
            EnterpriseRiskList.put("filterColumnAlphaNumericCount", filterColAlphaNumericCount);
        } else {
            List<EnterpriseRisk> allDocuments = new ArrayList<>();
            ...
            String template = "ENTERPRISE_RISK_TEMPLATE";
            ...
            EnterpriseRiskList.put("totalPage", totalPages);
            EnterpriseRiskList.put("count", totalCount);
            EnterpriseRiskList.put("pageScroll", pageScroll);
            EnterpriseRiskList.put("content", allDocuments);
            EnterpriseRiskList.put("sorting", document);
            EnterpriseRiskList.put("pitId", pitId);
        }
        return EnterpriseRiskList;
    }
    ...
    public CompletableFuture<Void> getEnterpriseRiskListExport(String reportDate, String fileName, List<FieldMap> fieldMaps, String colour, List<String> fieldColumns, DownloadJob job, String outputFormat, String sortBy, String sortingOrder, String userName, int pageSize, boolean globalSearch, String bulkFilterSearchType, ElasticSearchModel elasticSearchModel) throws Exception {
        ...
        for (int i = 0; i < totalPage; i++) {
            ...
            Document searchEnterpriseRisk = getEnterpriseRiskList(reportDate,
            ...
            EnterpriseRiskList.addAll((Collection<? extends EnterpriseRisk>) searchEnterpriseRisk.get("content"));
            docs = (Document) searchEnterpriseRisk.get("sorting");

            downloadJobService.updateJobProgress("FETCH_ITEMS", (int) totalCount, EnterpriseRiskList.size(), job.getId());
            totalPage = (long) searchEnterpriseRisk.get("totalPage");
            log.info("TOTAL PAGE {}", totalPage);
            totalCount = (long) searchEnterpriseRisk.get("count");
            log.info("TOTAL COUNT {}", totalCount);
        }
        log.info(" Export: Fetch complete.");
        downloadJobService.updateJobProgress("FILE_WRITE", EnterpriseRiskList.size(), (int) totalCount, job.getId());
        return elastic_utils.exportFileFromDetails(outputFormat, EnterpriseRiskList, fileName, fieldMaps, colour, job, fieldColumns, elasticSearchModel);
    }
}
