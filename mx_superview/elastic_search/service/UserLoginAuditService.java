...
import com.finsurge.tmr_portal.mx_superview.elastic_search.entity.UserLoginAudit;
...
public class UserLoginAuditService {
    private static final Logger log = LoggerFactory.getLogger(UserLoginAuditService.class);
    @Autowired
    ...
    public UserLoginAuditService(SearchHistoryRepository searchHistoryRepository, Environment environment, ElasticsearchClient esClient) {
        this.searchHistoryRepository = searchHistoryRepository;
        this.environment = environment;
        this.esClient = esClient;
    }

    public Document getUserLoginAuditList(String toDate, String fromDate, ElasticSearchModel elasticSearchModel, String sortBy, String sortingOrder,
                                          int pageSize, boolean isGlobalSearch, String bulkFilterSearchType, String username) throws IOException, ParseException {

        Document UserLoginAuditList = new Document();
        List<Query> queryList = new ArrayList<>();
        String indexName = UserLoginAudit.INDEX_NAME;

        // Fetching Data From  JSON File .
        List<String> fields = elastic_utils.getDataFromJsonFile("elastic/GlobalSearchFields/userLoginAudit.json", "fields");
        //Fetching IntegerFields From  JSON File .
        List<String> integerFields = elastic_utils.getDataFromJsonFile("elastic/GlobalSearchFields/userLoginAudit.json", "integerFields");
        //Fetching Global Search Fields From  JSON File .
        List<String> globalSearchFields = elastic_utils.getDataFromJsonFile("elastic/GlobalSearchFields/userLoginAudit.json", "globalSearchFields");
        ...
        if (fromDate == null || fromDate.isEmpty()) {
            fromDate = toDate; // Assign reportDate (toDate) to fromDate if fromDate is empty
        }
        ...
        Query dateRangeQuery = RangeQuery.of(r -> r
                .field("reportDate")
                .gte(JsonData.of(finalFromDate))  // Greater than or equal to fromDate
                .lte(JsonData.of(toDate))    // Less than or equal to toDate
        )._toQuery();
        queryList.add(dateRangeQuery);
        ...
        if (elasticSearchModel.getFilterColumnName() != null && !elasticSearchModel.getFilterColumnName().isEmpty()) {
            ...
            SearchResponse<UserLoginAudit> searchResponse = esClient.search(searchRequest, UserLoginAudit.class);
            ...
            dropDownFieldCount = elastic_utils.getDropDownFieldCount(elasticSearchModel.getFilterColumnName(), queryList, indexName, UserLoginAudit.class);
            ...
            UserLoginAuditList.put("filterValues", newDropDown);
            log.info("DropDown Count------------Cardinality :{}", dropDownFieldCount);
            UserLoginAuditList.put("dropDownFieldCount", dropDownFieldCount);
            UserLoginAuditList.put("filterColumnNumericCount", filterColNumericCount);
            UserLoginAuditList.put("filterColumnAlphaNumericCount", filterColAlphaNumericCount);
        } else {
            List<UserLoginAudit> allDocuments = new ArrayList<>();
            ...
            SortOptions sortOptions2 = SortOptions.of(s ->
                    s.field(FieldSort.of(fs -> fs.
                            field("auditId.keyword").order(sortingOrder.equalsIgnoreCase("asc")
            ...
            String template = "USER_LOGIN_AUDIT_TEMPLATE";
            ...
            UserLoginAuditList.put("totalPage", totalPages);
            UserLoginAuditList.put("count", totalCount);
            UserLoginAuditList.put("pageScroll", pageScroll);
            UserLoginAuditList.put("content", allDocuments);
            UserLoginAuditList.put("sorting", document);
            UserLoginAuditList.put("pitId", pitId);
        }
        return UserLoginAuditList;
    }


    @Async
    @Transactional
    public CompletableFuture<Void> getUserLoginAuditListExport(String reportDate, String fromDate, String fileName, List<FieldMap> fieldMaps, String colour, List<String> fieldColumns, DownloadJob job, String outputFormat, String sortBy, String sortingOrder, String userName, int pageSize, boolean globalSearch, String bulkFilterSearchType, ElasticSearchModel elasticSearchModel) throws Exception {
        List<UserLoginAudit> UserLoginAuditList = new ArrayList<>();
        ...
        for (int i = 0; i < totalPage; i++) {
            ...
            Document searchUserLoginAudit = getUserLoginAuditList(reportDate, fromDate,
            ...
            UserLoginAuditList.addAll((Collection<? extends UserLoginAudit>) searchUserLoginAudit.get("content"));
            docs = (Document) searchUserLoginAudit.get("sorting");

            downloadJobService.updateJobProgress("FETCH_ITEMS", (int) totalCount, UserLoginAuditList.size(), job.getId());
            totalPage = (long) searchUserLoginAudit.get("totalPage");
            log.info("TOTAL PAGE {}", totalPage);
            totalCount = (long) searchUserLoginAudit.get("count");
            log.info("TOTAL COUNT {}", totalCount);
            countFetched = true;
//            page++;
        }
        log.info(" Export: Fetch complete.");
        downloadJobService.updateJobProgress("FILE_WRITE", UserLoginAuditList.size(), (int) totalCount, job.getId());
        return elastic_utils.exportFileFromDetails(outputFormat, UserLoginAuditList, fileName, fieldMaps, colour, job, fieldColumns, elasticSearchModel);
    }
}
