...
import com.finsurge.tmr_portal.mx_superview.elastic_search.entity.OperationRights;
...
@Service
public class OperationRightsService {
    private static final Logger log = LoggerFactory.getLogger(OperationRights.class);
    @Autowired
    ...
    public OperationRightsService(SearchHistoryRepository searchHistoryRepository, Environment environment, ElasticsearchClient esClient) {
        this.searchHistoryRepository = searchHistoryRepository;
        this.environment = environment;
        this.esClient = esClient;
    }

    public Document getOperationRightsList(String date, ElasticSearchModel elasticSearchModel, String sortBy, String sortingOrder, String operRgt,
                                           int pageSize, boolean isGlobalSearch, String bulkFilterSearchType, String username) throws IOException, ParseException {

        Document operationRightsList = new Document();
        List<Query> queryList = new ArrayList<>();
        String indexName = OperationRights.INDEX_NAME;

        // Fetching Data From  JSON File .
        List<String> fields = elastic_utils.getDataFromJsonFile("elastic/GlobalSearchFields/operationRights.json", "fields");
        //Fetching IntegerFields From  JSON File .
        List<String> integerFields = elastic_utils.getDataFromJsonFile("elastic/GlobalSearchFields/operationRights.json", "integerFields");
        //Fetching Global Search Fields From  JSON File .
        List<String> globalSearchFields = elastic_utils.getDataFromJsonFile("elastic/GlobalSearchFields/operationRights.json", "globalSearchFields");
        ...
        // Filter by tmplType
        if (operRgt != null && !operRgt.isEmpty()) {
            if (operRgt.equals("Netting")) {
                Query tmplTypeQuery = TermQuery.of(q -> q.field("operRgt").value("nkey"))._toQuery();
                queryList.add(tmplTypeQuery);
            } else if (operRgt.equals("Liquidation")) {
                Query tmplTypeQuery = TermQuery.of(q -> q.field("operRgt").value("lpos"))._toQuery();
                queryList.add(tmplTypeQuery);
            }
        }
        ...
        if (elasticSearchModel.getFilterColumnName() != null && !elasticSearchModel.getFilterColumnName().isEmpty()) {
            ...
            SearchResponse<OperationRights> searchResponse = esClient.search(searchRequest, OperationRights.class);
            ...
            dropDownFieldCount = elastic_utils.getDropDownFieldCount(elasticSearchModel.getFilterColumnName(), queryList, indexName, OperationRights.class);
            ...
            operationRightsList.put("filterValues", newDropDown);
            operationRightsList.put("dropDownFieldCount", dropDownFieldCount);
            operationRightsList.put("filterColumnNumericCount", filterColNumericCount);
            operationRightsList.put("filterColumnAlphaNumericCount", filterColAlphaNumericCount);
        } else {
            List<OperationRights> allDocuments = new ArrayList<>();
            ...
            String template = "OPERATION_RIGHTS_TEMPLATE";
            ...
            operationRightsList.put("totalPage", totalPages);
            operationRightsList.put("count", totalCount);
            operationRightsList.put("pageScroll", pageScroll);
            operationRightsList.put("content", allDocuments);
            operationRightsList.put("sorting", document);
            operationRightsList.put("pitId", pitId);
        }
        return operationRightsList;
    }

    @Async
    @Transactional
    public CompletableFuture<Void> getOperationRightsListExport(String reportDate, String fileName, List<FieldMap> fieldMaps, String colour, List<String> fieldColumns, DownloadJob job, String outputFormat, String sortBy, String sortingOrder, String operRgt, String userName, int pageSize, boolean globalSearch, String bulkFilterSearchType, ElasticSearchModel elasticSearchModel) throws Exception {
        List<OperationRights> operationRightsList = new ArrayList<>();
        ...
        for (int i = 0; i < totalPage; i++) {
            ...
            Document searchOperationRights = getOperationRightsList(reportDate,
                    elasticSearchModel, sortBy, sortingOrder, operRgt, pageSize, globalSearch, bulkFilterSearchType, userName);
            //   log.info("NEW RESPONSE :{}", searchGroupComboPortfolio);

            operationRightsList.addAll((Collection<? extends OperationRights>) searchOperationRights.get("content"));
            docs = (Document) searchOperationRights.get("sorting");

            downloadJobService.updateJobProgress("FETCH_ITEMS", (int) totalCount, operationRightsList.size(), job.getId());
            totalPage = (long) searchOperationRights.get("totalPage");
            log.info("TOTAL PAGE {}", totalPage);
            totalCount = (long) searchOperationRights.get("count");
            log.info("TOTAL COUNT {}", totalCount);
        }
        log.info(" Export: Fetch complete.");
        downloadJobService.updateJobProgress("FILE_WRITE", operationRightsList.size(), (int) totalCount, job.getId());
        return elastic_utils.exportFileFromDetails(outputFormat, operationRightsList, fileName, fieldMaps, colour, job, fieldColumns, elasticSearchModel);
    }

    @SneakyThrows
    public void updateGroupLabels(String reportDate, String indexName, String reportFieldName) {
        ...
        for (String fieldValue : sortedDistinctFieldValues.keySet()) {
            ...
            // Determine the appropriate fetchUserGroupFieldName based on the operRgt value
            String operRgtValue = getOperRgtValue(indexName, reportDate, fieldValue);
            String fetchUserGroupFieldName = "NKEY".equals(operRgtValue) ? "nkeyTmpl" : "lposRight";
    ...
    private String getOperRgtValue(String indexName, String reportDate, String fieldValue) {
        ...
        SearchResponse<OperationRights> searchResponse;
        try {
            searchResponse = esClient.search(searchRequest, OperationRights.class);
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
        if (!searchResponse.hits().hits().isEmpty()) {
            return Objects.requireNonNull(searchResponse.hits().hits().get(0).source()).getOperRgt();
        }
        return null;
    }
