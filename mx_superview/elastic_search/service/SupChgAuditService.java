...
import com.finsurge.tmr_portal.mx_superview.elastic_search.entity.SupChgAudit;
...
@Service
public class SupChgAuditService {
    private static final Logger log = LoggerFactory.getLogger(SupChgAuditService.class);
    @Autowired
    ...
    public SupChgAuditService(SearchHistoryRepository searchHistoryRepository, Environment environment, ElasticsearchClient esClient) {
        this.searchHistoryRepository = searchHistoryRepository;
        this.environment = environment;
        this.esClient = esClient;
    }

    public Document getSupChgAuditList(String toDate, String fromDate, ElasticSearchModel elasticSearchModel, String sortBy, String sortingOrder,
                                       int pageSize, boolean isGlobalSearch, String bulkFilterSearchType, String username) throws IOException, ParseException {

        Document SupChgAuditList = new Document();
        List<Query> queryList = new ArrayList<>();
        String indexName = SupChgAudit.INDEX_NAME;

        // Fetching Data From  JSON File .
        List<String> fields = elastic_utils.getDataFromJsonFile("elastic/GlobalSearchFields/supChgAudit.json", "fields");
        //Fetching IntegerFields From  JSON File .
        List<String> integerFields = elastic_utils.getDataFromJsonFile("elastic/GlobalSearchFields/supChgAudit.json", "integerFields");
        //Fetching Global Search Fields From  JSON File .
        List<String> globalSearchFields = elastic_utils.getDataFromJsonFile("elastic/GlobalSearchFields/supChgAudit.json", "globalSearchFields");
        ...
        if (elasticSearchModel.getFilterColumnName() != null && !elasticSearchModel.getFilterColumnName().isEmpty()) {
            ...
            SearchResponse<SupChgAudit> searchResponse = esClient.search(searchRequest, SupChgAudit.class);
            ...
            dropDownFieldCount = elastic_utils.getDropDownFieldCount(elasticSearchModel.getFilterColumnName(), queryList, indexName, SupChgAudit.class);
            ...
            SupChgAuditList.put("filterValues", newDropDown);
            log.info("DropDown Count------------Cardinality :{}", dropDownFieldCount);
            SupChgAuditList.put("dropDownFieldCount", dropDownFieldCount);
            SupChgAuditList.put("filterColumnNumericCount", filterColNumericCount);
            SupChgAuditList.put("filterColumnAlphaNumericCount", filterColAlphaNumericCount);
        } else {
            List<SupChgAudit> allDocuments = new ArrayList<>();
            ...
            String template = "SUP_CHG_AUDIT__TEMPLATE";
            ...
            SupChgAuditList.put("totalPage", totalPages);
            SupChgAuditList.put("count", totalCount);
            SupChgAuditList.put("pageScroll", pageScroll);
            SupChgAuditList.put("content", allDocuments);
            SupChgAuditList.put("sorting", document);
            SupChgAuditList.put("pitId", pitId);
        }
        return SupChgAuditList;
    }


    @Async
    @Transactional
    public CompletableFuture<Void> getSupChgAuditListExport(String reportDate, String fromDate, String fileName, List<FieldMap> fieldMaps, String colour, List<String> fieldColumns, DownloadJob job, String outputFormat, String sortBy, String sortingOrder, String userName, int pageSize, boolean globalSearch, String bulkFilterSearchType, ElasticSearchModel elasticSearchModel) throws Exception {
        List<SupChgAudit> supChgAuditList = new ArrayList<>();
        ...
        for (int i = 0; i < totalPage; i++) {
            ...
            Document searchSupChgAudit = getSupChgAuditList(reportDate, fromDate,
            ...
            supChgAuditList.addAll((Collection<? extends SupChgAudit>) searchSupChgAudit.get("content"));
            docs = (Document) searchSupChgAudit.get("sorting");

            downloadJobService.updateJobProgress("FETCH_ITEMS", (int) totalCount, supChgAuditList.size(), job.getId());
            totalPage = (long) searchSupChgAudit.get("totalPage");
            log.info("TOTAL PAGE {}", totalPage);
            totalCount = (long) searchSupChgAudit.get("count");
            log.info("TOTAL COUNT {}", totalCount);
        }
        log.info(" Export: Fetch complete.");
        downloadJobService.updateJobProgress("FILE_WRITE", supChgAuditList.size(), (int) totalCount, job.getId());
        return elastic_utils.exportFileFromDetails(outputFormat, supChgAuditList, fileName, fieldMaps, colour, job, fieldColumns, elasticSearchModel);
    }
}
