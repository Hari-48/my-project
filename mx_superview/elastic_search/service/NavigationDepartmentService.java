...
@Service
public class NavigationDepartmentService {

    private static final Logger log = LoggerFactory.getLogger(NavigationRightsDepartment.class);
    @Autowired
    ...
    public NavigationDepartmentService(SearchHistoryRepository searchHistoryRepository, Environment environment, ElasticsearchClient esClient) {
    ...
    @SneakyThrows
    public Document getNavigationDepartmentData(String date, ElasticSearchModel elasticSearchModel, String sortBy, String sortingOrder,
                                                int pageSize, boolean isGlobalSearch, String bulkFilterSearchType, String username) {

        CountRequest countRequest = CountRequest.of(c -> c.index(NavigationRightsDepartment.INDEX_NAME));

        long count = esClient.count(countRequest).count();

        log.info("count;{}", count);
        if (count == 0) {
            insertLatestNavigationActiveData();
        }

        Document navigationDepartment = new Document();

        List<Query> queryList = new ArrayList<>();
        String indexName = NavigationRightsDepartment.INDEX_NAME;

        List<String> fields = elastic_utils.getDataFromJsonFile("elastic/GlobalSearchFields/navigationRightsDepartment.json", "fields");
        List<String> integerFields = elastic_utils.getDataFromJsonFile("elastic/GlobalSearchFields/navigationRightsDepartment.json", "integerFields");
        List<String> globalSearchFields = elastic_utils.getDataFromJsonFile("elastic/GlobalSearchFields/navigationRightsDepartment.json", "globalSearchFields");
        ...
        if (elasticSearchModel.getFilterColumnName() != null && !elasticSearchModel.getFilterColumnName().isEmpty()) {
            ...
            SearchResponse<NavigationRightsDepartment> searchResponse = esClient.search(searchRequest, NavigationRightsDepartment.class);
            ...
            dropDownFieldCount = elastic_utils.getDropDownFieldCount(elasticSearchModel.getFilterColumnName(), queryList, indexName, NavigationRightsDepartment.class);
            ...
            navigationDepartment.put("filterValues", newDropDown);
            navigationDepartment.put("dropDownFieldCount", dropDownFieldCount);
            navigationDepartment.put("filterColumnNumericCount", filterColNumericCount);
            navigationDepartment.put("filterColumnAlphaNumericCount", filterColAlphaNumericCount);
        } else {
            List<NavigationRightsDepartment> allDocuments = new ArrayList<>();
            ...
            SortOptions sortOptions2 = SortOptions.of(s ->
                    s.field(FieldSort.of(fs -> fs.
                            field("path.keyword").order(sortingOrder.equalsIgnoreCase("asc")
            ...
            String template = "NAVIGATION_RIGHT_TEMPLATE";
            ...
            navigationDepartment.put("totalPage", totalPages);
            navigationDepartment.put("count", totalCount);
            navigationDepartment.put("pageScroll", pageScroll);
            navigationDepartment.put("content", allDocuments);
            navigationDepartment.put("sorting", document);
            navigationDepartment.put("pitId", pitId);
        }
        return navigationDepartment;
    }

    @Async
    @Transactional
    public CompletableFuture<Void> getNavigationDepartmentExport(String reportDate, String fileName, List<FieldMap> fieldMaps, String colour, List<String> fieldColumns, DownloadJob job, String outputFormat, String sortBy, String sortingOrder, String userName, int pageSize, boolean globalSearch, String bulkFilterSearchType, ElasticSearchModel elasticSearchModel) {
        List<NavigationRightsDepartment> navigationRightsDepartmentList = new ArrayList<>();
        ...
        for (int i = 0; i < totalPage; i++) {
            ...
            Document navigationDepartmentData = getNavigationDepartmentData(reportDate,
            ...
            navigationRightsDepartmentList.addAll((Collection<? extends NavigationRightsDepartment>) navigationDepartmentData.get("content"));
            docs = (Document) navigationDepartmentData.get("sorting");

            downloadJobService.updateJobProgress("FETCH_ITEMS", (int) totalCount, navigationRightsDepartmentList.size(), job.getId());
            totalPage = (long) navigationDepartmentData.get("totalPage");
            log.info("TOTAL PAGE {}", totalPage);
            totalCount = (long) navigationDepartmentData.get("count");
            log.info("TOTAL COUNT {}", totalCount);
        }
        log.info(" Export: Fetch complete.");
        downloadJobService.updateJobProgress("FILE_WRITE", navigationRightsDepartmentList.size(), (int) totalCount, job.getId());
        return elastic_utils.exportFileFromDetails(outputFormat, navigationRightsDepartmentList, fileName, fieldMaps, colour, job, fieldColumns, elasticSearchModel);
    }

    public void insertLatestNavigationActiveData() throws IOException {
        // when Index is empty ,  I need to Update this  Query.( for only initial Stage )
        SearchRequest searchRequest = SearchRequest.of(s -> s.index(NavigationRightsDepartment.INDEX_NAME).size(10000));
        SearchResponse<NavigationRightsDepartment> searchResponse = esClient.search(searchRequest, NavigationRightsDepartment.class);
        log.info("Group Label Count : {}", searchResponse.hits().hits().size());
        if (searchResponse.hits().hits().isEmpty()) {
            log.info("Initial Update:-");
            String latestDate = null;
            SearchRequest groupNavigationList = SearchRequest.of(s -> s.index(GroupNavigationRights.INDEX_NAME).size(1).
                    sort(SortOptions.of(y -> y.field(FieldSort.of(f -> f.field("reportDate").order(SortOrder.Desc))))));
            SearchResponse<GroupNavigationRights> searchFirstDataResponse = esClient.search(groupNavigationList, GroupNavigationRights.class);
            ...
            List<String> uniqueList = groupNavigationRightsService.getPathField(reportDate);
            for (String unique : uniqueList) {
                executorService.submit(() -> {
                    try {
                        updateLatestData(unique, reportDate);
                    } catch (IOException e) {
    ...
    public void updateLatestData(String path, String reportDate) throws IOException {
        ...
        Query boolQuery = BoolQuery.of(b -> b.must(pathQuery).must(reportDateQuery).must(status))._toQuery();
        SearchRequest searchRequest = SearchRequest.of(s -> s.index(GroupNavigationRights.INDEX_NAME).query(boolQuery).size(1));
        SearchResponse<GroupNavigationRights> searchResponse = esClient.search(searchRequest, GroupNavigationRights.class);
        groupNavigationRightsService.insertMenuPath(searchResponse.hits().hits().stream().map(Hit::source).collect(Collectors.toList()));
    }
    public ResponseEntity<?> updateDepartmentField(String path, String name, Boolean userConfirmation, List<String> newRights) throws IOException {
        if (userConfirmation) {
            updateDepartments(path, newRights, name);
    ...
    @SneakyThrows
    public void updateDepartments(String path, List<String> newRights, String name) {
        Query pathQuery = TermQuery.of(t -> t.field("path.keyword").value(path))._toQuery();
        Query boolQuery = BoolQuery.of(b -> b.must(pathQuery))._toQuery();
        SearchRequest searchRequest = SearchRequest.of(s -> s.query(boolQuery).index(NavigationRightsDepartment.INDEX_NAME).size(10000));
        SearchResponse<NavigationRightsDepartment> searchResponse = esClient.search(searchRequest, NavigationRightsDepartment.class);
        if (!searchResponse.hits().hits().isEmpty()) {
            for (Hit<NavigationRightsDepartment> hit : searchResponse.hits().hits()) {
                String docId = hit.id();
                String updatedDate = String.valueOf(LocalDateTime.now());
                Map<String, Object> updateDepartment = new HashMap<>();
                updateDepartment.put("departmentAmendRights", newRights);
                updateDepartment.put("username", name);
                updateDepartment.put("updatedTime", updatedDate);
                UpdateRequest<Object, Object> updateRequest = UpdateRequest.of(u -> u.id(docId).index(NavigationRightsDepartment.INDEX_NAME).
                        retryOnConflict(3).
                        doc(updateDepartment));
                esClient.update(updateRequest, NavigationRightsDepartment.class);
            }
        }
    }
}
