...
import com.finsurge.tmr_portal.mx_superview.elastic_search.entity.StpRightsTypology;
...
@Service
public class StpRightsTypologyService {
    private static final Logger log = LoggerFactory.getLogger(StpRightsTypologyService.class);
    @Autowired
    ...
    public StpRightsTypologyService(SearchHistoryRepository searchHistoryRepository, Environment environment, ElasticsearchClient esClient) {
        this.searchHistoryRepository = searchHistoryRepository;
        this.environment = environment;
        this.esClient = esClient;
    }

    public Document getStpRightsTypologyList(String date, ElasticSearchModel elasticSearchModel, String sortBy, String sortingOrder,
                                             int pageSize, boolean isGlobalSearch,String bulkFilterSearchType, String username) throws IOException, ParseException {

        Document stpRightsTypologyList = new Document();
        List<Query> queryList = new ArrayList<>();
        String indexName = StpRightsTypology.INDEX_NAME;

        // Fetching Data From  JSON File .
        List<String> fields = elastic_utils.getDataFromJsonFile("elastic/GlobalSearchFields/stpRightsTypology.json", "fields");
        //Fetching IntegerFields From  JSON File .
        List<String> integerFields = elastic_utils.getDataFromJsonFile("elastic/GlobalSearchFields/stpRightsTypology.json", "integerFields");
        //Fetching Global Search Fields From  JSON File .
        List<String> globalSearchFields = elastic_utils.getDataFromJsonFile("elastic/GlobalSearchFields/stpRightsTypology.json", "globalSearchFields");
        ...
        if (elasticSearchModel.getFilterColumnName() != null && !elasticSearchModel.getFilterColumnName().isEmpty()) {
            ...
            SearchResponse<StpRightsTypology> searchResponse = esClient.search(searchRequest, StpRightsTypology.class);
            ...
            dropDownFieldCount = elastic_utils.getDropDownFieldCount(elasticSearchModel.getFilterColumnName(), queryList, indexName, StpRightsTypology.class);
            ...
            stpRightsTypologyList.put("filterValues", newDropDown);
            stpRightsTypologyList.put("dropDownFieldCount", dropDownFieldCount);
            stpRightsTypologyList.put("filterColumnNumericCount", filterColNumericCount);
            stpRightsTypologyList.put("filterColumnAlphaNumericCount", filterColAlphaNumericCount);
        } else {
            List<StpRightsTypology> allDocuments = new ArrayList<>();
            ...
            SortOptions sortOptions2 = SortOptions.of(s ->
                    s.field(FieldSort.of(fs -> fs.
                            field("typologyGroup.keyword").order(sortingOrder.equalsIgnoreCase("asc")
            ...
            String template = "STP_RIGHTS_TYPOLOGY_TEMPLATE";
            ...
            stpRightsTypologyList.put("totalPage", totalPages);
            stpRightsTypologyList.put("count", totalCount);
            stpRightsTypologyList.put("pageScroll", pageScroll);
            stpRightsTypologyList.put("content", allDocuments);
            stpRightsTypologyList.put("sorting", document);
            stpRightsTypologyList.put("pitId", pitId);
        }
        return stpRightsTypologyList;
    }

    @Async
    @Transactional
    public CompletableFuture<Void> getStpRightsTypologyListExport(String reportDate, String fileName, List<FieldMap> fieldMaps, String colour, List<String> fieldColumns, DownloadJob job, String outputFormat, String sortBy, String sortingOrder, String userName, int pageSize, boolean globalSearch,String bulkFilterSearchType, ElasticSearchModel elasticSearchModel) throws Exception {
        List<StpRightsTypology> stpRightsTypologyList = new ArrayList<>();
        ...
        for (int i = 0; i < totalPage; i++) {
            ...
            Document searchStpRightsTypology = getStpRightsTypologyList(reportDate,
            ...
            stpRightsTypologyList.addAll((Collection<? extends StpRightsTypology>) searchStpRightsTypology.get("content"));
            docs = (Document) searchStpRightsTypology.get("sorting");

            downloadJobService.updateJobProgress("FETCH_ITEMS", (int) totalCount, stpRightsTypologyList.size(), job.getId());
            totalPage = (long) searchStpRightsTypology.get("totalPage");
            log.info("TOTAL PAGE {}", totalPage);
            totalCount = (long) searchStpRightsTypology.get("count");
...
//            page++;
        ...
        return elastic_utils.exportFileFromDetails(outputFormat, stpRightsTypologyList, fileName, fieldMaps, colour, job, fieldColumns, elasticSearchModel);
    }
}
