...
import com.finsurge.tmr_portal.mx_superview.elastic_search.entity.DisplaySI;
...
@Service
public class DisplaySiService {
    private static final Logger log = LoggerFactory.getLogger(DisplaySiService.class);
    @Autowired
    ...
    public DisplaySiService(SearchHistoryRepository searchHistoryRepository, Environment environment, ElasticsearchClient esClient) {
        this.searchHistoryRepository = searchHistoryRepository;
        this.environment = environment;
        this.esClient = esClient;
    }

    public Document getDisplaySiList(String date, ElasticSearchModel elasticSearchModel, String sortBy, String sortingOrder, String tmplType,
                                     int pageSize, boolean isGlobalSearch, String bulkFilterSearchType, String username) throws IOException {

        Document displaySiList = new Document();
        List<Query> queryList = new ArrayList<>();
        String indexName = DisplaySI.INDEX_NAME;

        // Fetching Data From  JSON File .
        List<String> fields = elastic_utils.getDataFromJsonFile("elastic/GlobalSearchFields/displaySi.json", "fields");
        //Fetching IntegerFields From  JSON File .
        List<String> integerFields = elastic_utils.getDataFromJsonFile("elastic/GlobalSearchFields/displaySi.json", "integerFields");
        //Fetching Global Search Fields From  JSON File .
        List<String> globalSearchFields = elastic_utils.getDataFromJsonFile("elastic/GlobalSearchFields/displaySi.json", "globalSearchFields");
        ...
        if (elasticSearchModel.getFilterColumnName() != null && !elasticSearchModel.getFilterColumnName().isEmpty()) {
            ...
            SearchResponse<DisplaySI> searchResponse = esClient.search(searchRequest, DisplaySI.class);
            ...
            dropDownFieldCount = elastic_utils.getDropDownFieldCount(elasticSearchModel.getFilterColumnName(), queryList, indexName, DisplaySI.class);
            ...
            displaySiList.put("filterValues", newDropDown);
            log.info("DropDown Count------------Cardinality :{}", dropDownFieldCount);
            displaySiList.put("dropDownFieldCount", dropDownFieldCount);
            displaySiList.put("filterColumnNumericCount", filterColNumericCount);
            displaySiList.put("filterColumnAlphaNumericCount", filterColAlphaNumericCount);
        } else {
            List<DisplaySI> allDocuments = new ArrayList<>();
            ...
            String template = "DISPLAY_SI_TEMPLATE";
            ...
            displaySiList.put("totalPage", totalPages);
            displaySiList.put("count", totalCount);
            displaySiList.put("pageScroll", pageScroll);
            displaySiList.put("content", allDocuments);
            displaySiList.put("sorting", document);
            displaySiList.put("pitId", pitId);
        }
        return displaySiList;
    }

    @Async
    @Transactional
    public CompletableFuture<Void> getDisplaySiListExport(String reportDate, String fileName, List<FieldMap> fieldMaps, String colour, List<String> fieldColumns, DownloadJob job, String outputFormat, String sortBy, String sortingOrder, String tmplType, String userName, int pageSize, boolean globalSearch, String bulkFilterSearchType, ElasticSearchModel elasticSearchModel) throws Exception {
        List<DisplaySI> displaySIList = new ArrayList<>();
        ...
        for (int i = 0; i < totalPage; i++) {
            ...
            Document searchDisplaySi = getDisplaySiList(reportDate,
            ...
            displaySIList.addAll((Collection<? extends DisplaySI>) searchDisplaySi.get("content"));
            docs = (Document) searchDisplaySi.get("sorting");

            downloadJobService.updateJobProgress("FETCH_ITEMS", (int) totalCount, displaySIList.size(), job.getId());
            totalPage = (long) searchDisplaySi.get("totalPage");
            log.info("TOTAL PAGE {}", totalPage);
            totalCount = (long) searchDisplaySi.get("count");
            log.info("TOTAL COUNT {}", totalCount);
        }
        log.info(" Export: Fetch complete.");
        downloadJobService.updateJobProgress("FILE_WRITE", displaySIList.size(), (int) totalCount, job.getId());
        return elastic_utils.exportFileFromDetails(outputFormat, displaySIList, fileName, fieldMaps, colour, job, fieldColumns, elasticSearchModel);
    }
