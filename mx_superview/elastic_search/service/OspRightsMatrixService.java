...
import com.finsurge.tmr_portal.mx_superview.elastic_search.entity.OspRightsMatrix;
...
@Service
public class OspRightsMatrixService {
    private static final Logger log = LoggerFactory.getLogger(OspRightsMatrixService.class);
    @Autowired
    ...
    public OspRightsMatrixService(SearchHistoryRepository searchHistoryRepository, Environment environment, ElasticsearchClient esClient) {
        this.searchHistoryRepository = searchHistoryRepository;
        this.environment = environment;
        this.esClient = esClient;
    }

    public Document getOspRightsMatrixList(String date, ElasticSearchModel elasticSearchModel, String sortBy, String sortingOrder, Boolean inActiveTemplate,
                                           int pageSize, boolean isGlobalSearch, String bulkFilterSearchType, String username) throws IOException, ParseException {

        Document ospRightsMatrixList = new Document();
        List<Query> queryList = new ArrayList<>();
        String indexName = OspRightsMatrix.INDEX_NAME;

        // Fetching Data From  JSON File .
        List<String> fields = elastic_utils.getDataFromJsonFile("elastic/GlobalSearchFields/ospRightsMatrix.json", "fields");
        //Fetching IntegerFields From  JSON File .
        List<String> integerFields = elastic_utils.getDataFromJsonFile("elastic/GlobalSearchFields/ospRightsMatrix.json", "integerFields");
        //Fetching Global Search Fields From  JSON File .
        List<String> globalSearchFields = elastic_utils.getDataFromJsonFile("elastic/GlobalSearchFields/ospRightsMatrix.json", "globalSearchFields");
        ...
        if (elasticSearchModel.getFilterColumnName() != null && !elasticSearchModel.getFilterColumnName().isEmpty()) {
            ...
            SearchResponse<OspRightsMatrix> searchResponse = esClient.search(searchRequest, OspRightsMatrix.class);
            ...
            dropDownFieldCount = elastic_utils.getDropDownFieldCount(elasticSearchModel.getFilterColumnName(), queryList, indexName, OspRightsMatrix.class);
            ...
            ospRightsMatrixList.put("filterValues", newDropDown);
            ospRightsMatrixList.put("dropDownFieldCount", dropDownFieldCount);
            ospRightsMatrixList.put("filterColumnNumericCount", filterColNumericCount);
            ospRightsMatrixList.put("filterColumnAlphaNumericCount", filterColAlphaNumericCount);
        } else {
            List<OspRightsMatrix> allDocuments = new ArrayList<>();
            ...
            SortOptions sortOptions2 = SortOptions.of(s ->
                    s.field(FieldSort.of(fs -> fs.
                            field("ospRightTemplate.keyword").order(sortingOrder.equalsIgnoreCase("asc")
            ...
            String template = "OSP_RIGHTS_MATRIX_TEMPLATE";
            ...
            ospRightsMatrixList.put("totalPage", totalPages);
            ospRightsMatrixList.put("count", totalCount);
            ospRightsMatrixList.put("pageScroll", pageScroll);
            ospRightsMatrixList.put("content", allDocuments);
            ospRightsMatrixList.put("sorting", document);
            ospRightsMatrixList.put("pitId", pitId);
        }
        return ospRightsMatrixList;
    }

    @Async
    @Transactional
    public CompletableFuture<Void> getOspRightsMatrixListExport(String reportDate, String fileName, List<FieldMap> fieldMaps, String colour, List<String> fieldColumns, DownloadJob job, String outputFormat, String sortBy, String sortingOrder,Boolean inActiveTemplate, String userName, int pageSize, boolean globalSearch, String bulkFilterSearchType, ElasticSearchModel elasticSearchModel) throws Exception {
        List<OspRightsMatrix> ospRightsMatrixList = new ArrayList<>();
        ...
        for (int i = 0; i < totalPage; i++) {
            ...
            Document searchOspRightsMatrix = getOspRightsMatrixList(reportDate,
            ...
            ospRightsMatrixList.addAll((Collection<? extends OspRightsMatrix>) searchOspRightsMatrix.get("content"));
            docs = (Document) searchOspRightsMatrix.get("sorting");

            downloadJobService.updateJobProgress("FETCH_ITEMS", (int) totalCount, ospRightsMatrixList.size(), job.getId());
            totalPage = (long) searchOspRightsMatrix.get("totalPage");
            log.info("TOTAL PAGE {}", totalPage);
            totalCount = (long) searchOspRightsMatrix.get("count");
            log.info("TOTAL COUNT {}", totalCount);
        }
        log.info(" Export: Fetch complete.");
        downloadJobService.updateJobProgress("FILE_WRITE", ospRightsMatrixList.size(), (int) totalCount, job.getId());
        return elastic_utils.exportFileFromDetails(outputFormat, ospRightsMatrixList, fileName, fieldMaps, colour, job, fieldColumns, elasticSearchModel);
    }
}
