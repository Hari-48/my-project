...
import com.finsurge.tmr_portal.mx_superview.elastic_search.entity.StpRightsSrcMod;
...
@Service
public class StpRightsSrcModService {
    private static final Logger log = LoggerFactory.getLogger(StpRightsSrcModService.class);
    @Autowired
    ...
    public StpRightsSrcModService(SearchHistoryRepository searchHistoryRepository, Environment environment, ElasticsearchClient esClient) {
        this.searchHistoryRepository = searchHistoryRepository;
        this.environment = environment;
        this.esClient = esClient;
    }

    public Document getStpRightsSrcModList(String date, ElasticSearchModel elasticSearchModel, String sortBy, String sortingOrder,
                                           int pageSize, boolean isGlobalSearch,String bulkFilterSearchType, String username) throws IOException, ParseException {

        Document stpRightsSrcModList = new Document();
        List<Query> queryList = new ArrayList<>();
        String indexName = StpRightsSrcMod.INDEX_NAME;

        // Fetching Data From  JSON File .
        List<String> fields = elastic_utils.getDataFromJsonFile("elastic/GlobalSearchFields/stpRightsSrcMod.json", "fields");
        //Fetching IntegerFields From  JSON File .
        List<String> integerFields = elastic_utils.getDataFromJsonFile("elastic/GlobalSearchFields/stpRightsSrcMod.json", "integerFields");
        //Fetching Global Search Fields From  JSON File .
        List<String> globalSearchFields = elastic_utils.getDataFromJsonFile("elastic/GlobalSearchFields/stpRightsSrcMod.json", "globalSearchFields");
        ...
        if (elasticSearchModel.getFilterColumnName() != null && !elasticSearchModel.getFilterColumnName().isEmpty()) {
            ...
            SearchResponse<StpRightsSrcMod> searchResponse = esClient.search(searchRequest, StpRightsSrcMod.class);
            ...
            dropDownFieldCount = elastic_utils.getDropDownFieldCount(elasticSearchModel.getFilterColumnName(), queryList, indexName, StpRightsSrcMod.class);
            ...
            stpRightsSrcModList.put("filterValues", newDropDown);
            stpRightsSrcModList.put("dropDownFieldCount", dropDownFieldCount);
            stpRightsSrcModList.put("filterColumnNumericCount", filterColNumericCount);
            stpRightsSrcModList.put("filterColumnAlphaNumericCount", filterColAlphaNumericCount);
        } else {
            List<StpRightsSrcMod> allDocuments = new ArrayList<>();
            ...
            SortOptions sortOptions2 = SortOptions.of(s ->
                    s.field(FieldSort.of(fs -> fs.
                            field("processingCenter.keyword").order(sortingOrder.equalsIgnoreCase("asc")
            ...
            String template = "STP_RIGHTS_SRC_MOD_TEMPLATE";
            ...
            stpRightsSrcModList.put("totalPage", totalPages);
            stpRightsSrcModList.put("count", totalCount);
            stpRightsSrcModList.put("pageScroll", pageScroll);
            stpRightsSrcModList.put("content", allDocuments);
            stpRightsSrcModList.put("sorting", document);
            stpRightsSrcModList.put("pitId", pitId);
        }
        return stpRightsSrcModList;
    }

    @Async
    @Transactional
    public CompletableFuture<Void> getStpRightsSrcModListExport(String reportDate, String fileName, List<FieldMap> fieldMaps, String colour, List<String> fieldColumns, DownloadJob job, String outputFormat, String sortBy, String sortingOrder, String userName, int pageSize, boolean globalSearch,String bulkFilterSearchType, ElasticSearchModel elasticSearchModel) throws Exception {
        List<StpRightsSrcMod> stpRightsSrcModList = new ArrayList<>();
        ...
        for (int i = 0; i < totalPage; i++) {
            ...
            Document searchStpRightsSrcMod = getStpRightsSrcModList(reportDate,
            ...
            stpRightsSrcModList.addAll((Collection<? extends StpRightsSrcMod>) searchStpRightsSrcMod.get("content"));
            docs = (Document) searchStpRightsSrcMod.get("sorting");

            downloadJobService.updateJobProgress("FETCH_ITEMS", (int) totalCount, stpRightsSrcModList.size(), job.getId());
            totalPage = (long) searchStpRightsSrcMod.get("totalPage");
            log.info("TOTAL PAGE {}", totalPage);
            totalCount = (long) searchStpRightsSrcMod.get("count");
            log.info("TOTAL COUNT {}", totalCount);
            countFetched = true;
//            page++;
        }
        log.info(" Export: Fetch complete.");
        downloadJobService.updateJobProgress("FILE_WRITE", stpRightsSrcModList.size(), (int) totalCount, job.getId());
        return elastic_utils.exportFileFromDetails(outputFormat, stpRightsSrcModList, fileName, fieldMaps, colour, job, fieldColumns, elasticSearchModel);
    }
}
