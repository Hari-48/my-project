...
import com.finsurge.tmr_portal.mx_superview.elastic_search.service.ConfigMgtRightService;
...
@RestController
@CrossOrigin
public class ConfigMgtRightController {
    @Autowired
    private DownloadJobService downloadJobService;
    @Autowired
    private AuditUtils auditUtils;
    @Autowired
    private ConfigMgtRightService configMgtRightService;

    @PostMapping("/api/uam/config-mgt-right/filter")
    public ResponseEntity<?> configMgtRightList(
        ...
        Document configMgtRightList;

        if (isExport) {
            DownloadJob job = downloadJobService.createNewJob("CONFIGURATION_MANAGEMENT_RIGHTS", authentication.getName());
            ...
            configMgtRightService.getConfigMgtRightListExport(reportDate, "Configuration_Management_Rights_Template", elasticSearchModel.fieldMaps,
        ...
        configMgtRightList = configMgtRightService.getConfigMgtRightList(reportDate,
                elasticSearchModel, sortBy, sortingOrder, pageSize, isGlobalSearch, bulkFilterSearchType, authentication.getName());
        return new ResponseEntity<>(configMgtRightList, HttpStatus.OK);
    }
}
