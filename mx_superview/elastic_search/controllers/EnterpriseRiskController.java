...
import com.finsurge.tmr_portal.mx_superview.elastic_search.service.EnterpriseRiskService;
...
public class EnterpriseRiskController {
    ...
    @PostMapping("/api/uam/enterprise-risk/filter")
    public ResponseEntity<?> enterpriseRiskList(
        ...
        Document enterpriseRiskList;

        if (isExport) {
            DownloadJob job = downloadJobService.createNewJob("ENTERPRISE_RISK", authentication.getName());
            ...
            enterpriseRiskService.getEnterpriseRiskListExport(reportDate, "Enterprise_Risk_Template", elasticSearchModel.fieldMaps,
        ...
        enterpriseRiskList = enterpriseRiskService.getEnterpriseRiskList(reportDate,
                elasticSearchModel, sortBy, sortingOrder, pageSize, isGlobalSearch, bulkFilterSearchType, authentication.getName());
        return new ResponseEntity<>(enterpriseRiskList, HttpStatus.OK);
    }
}
