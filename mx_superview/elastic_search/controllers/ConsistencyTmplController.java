...
import com.finsurge.tmr_portal.mx_superview.elastic_search.service.ConsistencyTmplService;
...
@RestController
@CrossOrigin
public class ConsistencyTmplController {
    @Autowired
    private DownloadJobService downloadJobService;
    @Autowired
    private AuditUtils auditUtils;
    @Autowired
    private ConsistencyTmplService consistencyTmplService;

    @PostMapping("/api/uam/consistency-tmpl/filter")
    public ResponseEntity<?> getConsistencyTmplList(
            @RequestParam(defaultValue = "20") int pageSize,
            @RequestParam(defaultValue = "id") String sortBy,
            @RequestParam(defaultValue = "consistencyTmpl") String sortingOrder,
        ...
        Document consistencyTmplList;

        if (isExport) {
            DownloadJob job = downloadJobService.createNewJob("CONSISTENCY_TEMPLATE", authentication.getName());
            ...
            consistencyTmplService.getConsistencyTmplListExport(reportDate, "Consistency_Template", elasticSearchModel.fieldMaps,
        ...
        consistencyTmplList = consistencyTmplService.getConsistencyTmplList(reportDate,
        ...
        return new ResponseEntity<>(consistencyTmplList, HttpStatus.OK);
    }
}
