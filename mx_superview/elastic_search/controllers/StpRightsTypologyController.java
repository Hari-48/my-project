...
import com.finsurge.tmr_portal.mx_superview.elastic_search.service.StpRightsTypologyService;
...
@RestController
@CrossOrigin
public class StpRightsTypologyController {
    @Autowired
    private DownloadJobService downloadJobService;
    @Autowired
    private AuditUtils auditUtils;
    @Autowired
    private StpRightsTypologyService stpRightsTypologyService;

    @PostMapping("/api/uam/stp-rights-typology/filter")
    public ResponseEntity<?> getStpRightsTypology(
            @RequestParam(defaultValue = "20") int pageSize,
            @RequestParam(defaultValue = "id") String sortBy,
            @RequestParam(defaultValue = "typologyGroup") String sortingOrder,
        ...
        Document stpRightsTypologyList;
        ...
        if (isExport) {
            ...
            CompletableFuture<Void> future = stpRightsTypologyService.getStpRightsTypologyListExport(reportDate, "Stp_Rights_Typology_Template",elasticSearchModel.fieldMaps,
        ...
        stpRightsTypologyList = stpRightsTypologyService.getStpRightsTypologyList(reportDate,
                elasticSearchModel, sortBy, sortingOrder, pageSize, isGlobalSearch, bulkFilterSearchType,authentication.getName());
        return new ResponseEntity<>(stpRightsTypologyList, HttpStatus.OK);
    }
}
