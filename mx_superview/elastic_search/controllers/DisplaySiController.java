...
import com.finsurge.tmr_portal.mx_superview.elastic_search.service.DisplaySiService;
...
public class DisplaySiController {
    @Autowired
    private DownloadJobService downloadJobService;
    @Autowired
    private AuditUtils auditUtils;
    @Autowired
    private DisplaySiService displaySiService;

    @PostMapping("/api/uam/display-si/filter")
    public ResponseEntity<?> getFinanceRightsList(
        ...
        Document displaySiList;

        if (isExport) {
            DownloadJob job = downloadJobService.createNewJob("DISPLAY_SI", authentication.getName());
            ...
            displaySiService.getDisplaySiListExport(reportDate, "Display_Si_Template", elasticSearchModel.fieldMaps,
