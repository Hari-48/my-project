...
import com.finsurge.tmr_portal.mx_superview.elastic_search.service.OperationRightsService;
...
@RestController
@CrossOrigin
public class OperationRightsController {
    @Autowired
    ...
    @PostMapping("/api/uam/operation-rights/filter")
    public ResponseEntity<?> getOperationRights(
            @RequestParam(defaultValue = "20") int pageSize,
            @RequestParam(defaultValue = "id") String sortBy,
            @RequestParam(defaultValue = "template") String sortingOrder,
            @RequestParam(required = false) String operRgt,
        ...
        Document operationRightsList;

        if (isExport) {
            DownloadJob job = downloadJobService.createNewJob("OPERATION_RIGHTS", authentication.getName());
            ...
            operationRightsService.getOperationRightsListExport(reportDate, "Operation_Rights_Template", elasticSearchModel.fieldMaps,
                    elasticSearchModel.color, elasticSearchModel.fieldColumns, job, outputFormat, sortBy, sortingOrder, operRgt, authentication.getName(), pageSize, isGlobalSearch, bulkFilterSearchType, elasticSearchModel);
        ...
        operationRightsList = operationRightsService.getOperationRightsList(reportDate,
                elasticSearchModel, sortBy, sortingOrder, operRgt, pageSize, isGlobalSearch, bulkFilterSearchType, authentication.getName());
        return new ResponseEntity<>(operationRightsList, HttpStatus.OK);
    }
}
